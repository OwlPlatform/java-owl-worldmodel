...
public class StopTransientDecoder implements MessageDecoder {

	private static final Logger log = LoggerFactory
			.getLogger(StopTransientDecoder.class);
	...
	public MessageDecoderResult decodable(IoSession session, IoBuffer buffer) {
		if (buffer.prefixedDataAvailable(4, 65536)) {
			...
			buffer.reset();
			if (messageType == StopTransientMessage.MESSAGE_TYPE) {
	...
	public MessageDecoderResult decode(IoSession session, IoBuffer buffer,
		...
		StopTransientMessage message = new StopTransientMessage();
